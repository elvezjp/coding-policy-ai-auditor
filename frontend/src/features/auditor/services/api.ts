// API service for auditor feature
...
): Promise<{ success: boolean; report?: string; auditMeta?: AuditResult['auditMeta']; error?: string }> {
  const response = await fetch(`${getBackendUrl()}/api/audit`, {
    method: 'POST',
