...
class TestOpenAIProviderExecuteAudit:
    """OpenAIProvider.execute_audit()のテスト（モック使用）"""

    @patch("app.services.openai_service.OpenAI")
    def test_ut_oai_001_execute_audit_success(self, mock_openai_class):
        """UT-OAI-001: 正常なリクエスト"""
        # モックの設定
        mock_client = MagicMock()
        mock_openai_class.return_value = mock_client
        ...
        mock_response.choices[0].message.content = "## 監査結果\n問題ありません。"
        ...
        assert result.auditMeta.provider == "openai"
        assert result.auditMeta.modelId == "gpt-4o"
        assert result.auditMeta.inputTokens == 1000
        assert result.auditMeta.outputTokens == 200

    @patch("app.services.openai_service.OpenAI")
    def test_execute_audit_auth_error(self, mock_openai_class):
        """認証エラー時"""
        mock_client = MagicMock()
        mock_openai_class.return_value = mock_client
        mock_client.chat.completions.create.side_effect = AuthenticationError(
