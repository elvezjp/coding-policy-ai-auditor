...
from app.services.auditor_excel_parser import parse_auditor_excel_spreadsheet
...
@router.post("/auditor-excel-spreadsheet", response_model=AuditorSpreadsheetResponse)
async def parse_auditor_excel_spreadsheet_api(file: UploadFile = File(...)):
    """
    ...
    if ext not in ["xlsx", "xls"]:
        return AuditorSpreadsheetResponse(
            success=False,
            sheets=[],
            filename=filename,
            error="対応していないファイル形式です。Excel (.xlsx, .xls) ファイルを選択してください。",
    ...
    # 解析実行
    result = parse_auditor_excel_spreadsheet(content, filename)
    ...
    sheets = [
        SpreadsheetSheetModel(
            ...
            columns=s["columns"],
            hyperlinks={
                k: CellHyperlinkModel(sheetName=v["sheetName"], cell=v["cell"])
                for k, v in s["hyperlinks"].items()
            },
        )
