...
- UT-BED-PROVIDER-004: execute_audit() - 正常なリクエスト（モック）
"""
...
class TestBedrockProviderExecuteAudit:
    """BedrockProvider.execute_audit()のテスト（モック使用）"""

    @patch("app.services.bedrock_service.boto3")
    def test_ut_bed_provider_004_execute_audit_success(self, mock_boto3):
        ...
        assert result.auditMeta.provider == "bedrock"
    ...
    @patch("app.services.bedrock_service.boto3")
    def test_execute_audit_client_error(self, mock_boto3):
        """ClientError時"""
        from botocore.exceptions import ClientError
